...
export function DeleteCategoryForm({ categoryId, className }: DeleteCategoryFormProps) {
    const router = useRouter()
    const [isOpen, setIsOpen] = useState(false)
    const toastIdRef = useRef<string | number | null>(null)
    const deleteCategoryById = deleteCategory.bind(null, categoryId)
    const [state, dispatch, isPending] = useActionState(deleteCategoryById, {
        errors: [],
        success: "",
    })
    ...
    function handleConfirmSubmit() {
        toastIdRef.current = toast.loading("Eliminando categoría...")
    }
            ...
            {isOpen ? (
                ...
                >
                    <div
                        ...
                        className="w-full max-w-sm rounded-2xl border border-cielo-100 bg-white shadow-[0_25px_60px_-15px_rgba(15,23,42,0.3)]"
                    ...
                    >
                        <div className="flex flex-col items-center px-6 pt-7 pb-2">
                            ...
                            <p className="mt-1.5 text-center text-sm text-cielo-500">
                                Si la categoría tiene productos asociados, no podrá eliminarse. ¿Deseas continuar?
                            </p>
                        </div>
