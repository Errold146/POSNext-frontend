...
import { Coupon, CouponResSchema, Product, ShoppingCart } from "./schemas";
...
export const useStore = create<Store>()(devtools((set, get) => ({
    ...
    removeFromCart: (id) => {
        set((state) => ({contents: state.contents.filter(item => item.productId !== id)}))
    ...
    applyCoupon: async (couponName) => {
        ...
        const coupon = CouponResSchema.parse(json)
        set(() => ({ coupon }))
